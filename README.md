...
test/                             시험. npm test 는 DB 없이, npm run test:db 는 붙여서 돈다
  logic.ts                        점수 · 비밀번호 · 사진에서 온 것 거르기 (67개)
  db.ts                           수업 한 회차를 통째로 (77개) — 붙은 DB 의 LMS 표를 비운다
...
영역은 `src/config/lms.ts` 의 `AREAS` 한 배열이 단일 출처다. 하나를 더 열려면 거기 한 줄만
더하면 문항표 화면 · 집계 · 학생 화면이 함께 따라간다. 반대로 **이미 쓰인 영역 코드를 그냥
지우면 안 된다** — 그 코드로 저장된 과거 문항이 미아가 되어 채점에서 통째로 빠진다.
바꿔야 한다면 `0011` 처럼 저장된 줄을 함께 옮기는 마이그레이션을 같이 낸다.

새 회차에 미리 깔리는 배치도 같은 파일의 `DEFAULT_LAYOUT` 표 하나가 정한다. 화면에 보이는
"통상 배치(1–3 독서론 · …)" 안내 문구도 그 표에서 만들어 낸다 — 손으로 적어 두면 표를 고칠
때마다 안내가 조용히 거짓말이 된다.
...
**그래서 한 회차 안에 같은 번호가 두 줄 있는 것이 정상이다.** 35번은 화작 학생과 언매 학생이
서로 다른 문항을 푸는 자리다. 문항 하나를 가리키는 열쇠는 번호가 아니라 **번호 + 영역**이고,
`0008` 이 번호만으로 묶어 둔 것을 `0010` 이 고쳤다. 그 전에는 두 번째 벌이 저장할 때 조용히
버려져서 그 반의 언매 학생은 선택과목 11문항이 통째로 '아직 안 매김' 으로 남았다 —
화면 어디에도 그런 말이 없었다.

같은 번호에 **같은 영역**이 두 줄인 것은 여전히 실수라서 DB 가 막고, 편집 화면도 저장 전에 짚어 준다.
...
| `/lms/exams/[id]/questions` | 튜터 | 문항표 (사진에서 읽기 · 지난 회차에서 복사) |
...
회차마다 45줄을 손으로 채우는 것이 이 서비스에서 제일 지루한 일이다. 시험지 뒤의 정답표나
배점표를 찍어 올리면 초안을 만들어 준다 (`lib/lms/ocr.ts`, Claude Opus 5).

두 가지를 지킨다.

- **읽은 것만 말한다.** 사진에 영역이 안 적혀 있으면 지어내지 않고 비워서 온다. 비워 온 자리는
  앱이 통상 배치로 채우고, 화면이 "그중 N줄은 영역을 못 읽어 통상 배치로 채웠어요" 라고
  따로 말한다. 지어낸 값이 조용히 저장되는 것이 이 기능에서 가장 나쁜 일이다.
- **저장하지 않는다.** 결과는 편집기에 채워 넣기만 하고 튜터가 눈으로 보고 저장을 눌러야 DB 로
  간다. 사진에서 온 줄은 표에서 연한 색으로 표시된다. 사진 자체는 서버에 남기지 않는다.

구조화 출력(`output_config.format`)이 보장하는 것은 **키와 자료형까지**다. SDK 가 요청을 보내기
전에 `enum` 과 범위를 설명 문구로 낮추기 때문에, 모델이 목록에 없는 영역 코드나 6번 정답을
줄 수 있다. 그래서 값 검사는 `normalizeExtracted()` 가 다시 한다 — 거기가 진짜 방어선이고,
그래서 API 호출과 떼어 두었다(키 없이도 확인할 수 있다).

`ANTHROPIC_API_KEY` 가 없으면 이 기능만 꺼지고 나머지는 그대로 돈다.

### 되풀이를 줄이는 것들

실제로 수업을 돌려 보면 같은 동작을 사람 수만큼 반복하게 되는 자리가 생긴다.

- **회차 일괄 공개** — 학생이 열둘이면 채점 화면을 열두 번 들어가 공개를 눌러야 했다.
  버튼 하나로 바꾸되 **채점이 끝난 것만** 공개한다. 매기다 만 응시는 그대로 두고 몇 명이
  남았는지 말해 준다.
- **지난 회차에서 문항표 복사** — 같은 형식의 모의고사를 매주 보면 배점과 배치가 거의 그대로다.
  **정답은 가져오지 않는다** — 회차마다 반드시 다르고, 지난 회차 정답이 남아 있으면 고치는 걸
  잊었을 때 조용히 틀린 채점이 된다. 비워 오면 최소한 빈 게 보인다.
- **튜터가 자기 반 학생 비밀번호 재발급** — 관리자만 할 수 있게 두면 학생이 비번을 잊은 날
  수업이 멈춘다. 대신 `studentVisibleTo()` 로 자기 반 학생인지 반드시 확인한다.
- **성적 CSV** — 상담 자료는 화면 캡처가 아니라 손대서 정리할 수 있는 표여야 한다.
  맨 앞에 BOM 을 붙인다. 없으면 한국어 윈도우 엑셀이 EUC-KR 로 읽어서 이름이 전부 깨진다.
- **되돌릴 수 없는 것에는 확인을 붙였다.** 반·회차·계정 삭제와 문항표 새로 깔기가 확인 없이
  바로 실행되고 있었다. 무엇이 함께 사라지는지를 문구에 적는다 — "정말 삭제하시겠습니까?" 는
  아무 정보가 없어서 아무도 안 읽고 확인을 누른다.
...
**`npm test`** 는 순수 계산만 본다 — 점수를 어떻게 세는가, 비밀번호를 어떻게 다루는가,
사진에서 온 것을 어떻게 거르는가. 셋 다 틀리면 사람이 손해를 보는데 화면만 봐서는 틀린 줄
모르는 것들이다. 의존성이 없어서 아무 데서나 돈다.

**`npm run test:db`** 는 붙여 봐야 아는 것을 본다 — 제약이 실제로 거는가, 연쇄 삭제가 도는가,
저장이 실패했을 때 되돌아가는가. 실제로 이 시험을 만들다가 채점이 통째로 사라지는 버그를
찾았다(아래). 붙은 DB 의 **LMS 표를 비우므로** 평소 환경변수를 쓰지 않고 일부러 다른 이름을 본다.
...
자르기 때문이다. 응시 23개 × 답안 45개(1,035줄)에서 이미 채점이 끝난 응시가 '덜 매김' 으로
계산됐다. `inChunks()` 에 넘기는 조회는 겹치지 않는 열로 정렬해야 한다.
