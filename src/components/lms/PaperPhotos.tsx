...
export function PaperPhotos({
  ...
  async function send(key: string, blob: Blob) {
    ...
    try {
      ...
      if (out.ok) patch(key, { status: 'done', id: out.photo.id, blob: undefined });
      else patch(key, { status: 'error', error: UPLOAD_MESSAGES[out.reason] ?? UPLOAD_MESSAGES.FAILED });
    } catch {
  ...
  function drop(item: Item) {
    ...
    startTransition(async () => {
      const out = await remove(form).catch(() => ({ ok: false, reason: 'FAILED' }));
      if (out.ok) setItems((prev) => prev.filter((it) => it.key !== item.key));
      else setNotice(UPLOAD_MESSAGES[out.reason ?? 'FAILED'] ?? UPLOAD_MESSAGES.FAILED);
    });
  }
            ...
            <li key={item.key} className="flex flex-col gap-1">
              <div className="relative aspect-3/4 overflow-hidden rounded-xl bg-surface">
                ...
                {item.status === 'error' ? (
                  <button
                    type="button"
                    onClick={() => item.blob && void send(item.key, item.blob)}
