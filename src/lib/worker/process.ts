...
import { createNotionPage, notionConfigured, PDF_URL_TTL_SECONDS } from './notion';
import { mergeToPdf, pdfFileName } from './pdf';
  ...
  for (const subject of subjects) {
    ...
    if (!pdfPath && files.length > 0) {
      try {
        ...
        const path = `pdf/${receiptNo}/${pdfFileName(receiptNo, exam.grade, label)}`;
